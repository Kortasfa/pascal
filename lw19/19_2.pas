...
PROCEDURE CopyDateToT(VAR DateFile: FileOfDate; VAR TFile: FileOdDate);
VAR
  VarDate: Date;
BEGIN {CopyOut}
  WHILE NOT EOF(DateFile)
  DO
    BEGIN
      READ(DateFile, VarDate);
      WriteDate(TFile, VarDate);
      WRITELN;
    END
END;{CopyOut}

PROCEDURE CopeTtoDate(TFile, DateFile)(VAR TFile: FileOfDate; VAR DateFile: FileOdDate);
VAR
  VarDate: Date;
BEGIN {CopyOut}
  WHILE NOT EOF(DateFile)
    ...
    THEN
      BEGIN
        ...
        DO
          ...
          END; 
        WRITE(TFile, D);
        CopyDateToT(DateFile, TFile);
        CopeTtoDate(TFile, DateFile)
      END;
  RESET(DateFile);
  CopyOut(DateFile);
END.{SortDate}
